...
namespace util
{
	...
	void copy_command(CUserCmd* cmd, int tickbase_shift)
	{
		...
		else if (g_cfg.ragebot.slow_teleport)
		{
			if ((cmd->m_forwardmove) > 10.f)
				cmd->m_forwardmove = cmd->m_forwardmove;
			else if ((cmd->m_forwardmove) < -10.f)
				cmd->m_forwardmove = -cmd->m_forwardmove;

			if ((cmd->m_sidemove) > 10.f)
				cmd->m_sidemove = cmd->m_sidemove;
			else if ((cmd->m_sidemove) < -10.f)
				cmd->m_sidemove = -cmd->m_sidemove;
		}
		else {
