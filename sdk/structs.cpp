...
void entity_t::SetAngle2(const Vector& angle)
{
	if (!this) //-V704
		return;

	using Fn = void(__thiscall*)(void*, const Vector&);
	static auto fn = reinterpret_cast<Fn>(util::FindSignature(crypt_str("client.dll"), crypt_str("55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1")));
...
int weapon_t::get_max_tickbase_shift()
{
	...
	if (m_gamerules()->m_bIsValveDS())
		return max_tickbase_shift = min(max_tickbase_shift, 6);
...
float player_t::GetMaxBodyRotation() {
...
AnimationLayer* player_t::get_animlayers_resolver()
{
	return *(AnimationLayer**)((DWORD)this + 0x2980);
}

AnimationLayer* player_t::GetAnimOverlay_resolver(int index)
{
	if (index < 15)
...
float player_t::get_max_desync_delta_low()
{
	if (!this) //-V704
		return 0.0f;
	...
	return 30 * avg_speedfactor;
}

float player_t::get_max_desync_delta_mid()
{
	if (!this) //-V704
		return 0.0f;
	...
	return 37.5 * avg_speedfactor;
}

float player_t::get_max_desync_delta_air()
{
	if (!this) //-V704
		return 0.0f;
