...
public:
    ...
    void SetAngle2(const Vector& angle);
...
public:
    ...
    NETVAR(int, AddonBits, crypt_str("CCSPlayer"), crypt_str("m_iAddonBits"));
    ...
    int get_move_type();
    int animlayer_count();
    float GetMaxBodyRotation();
    AnimationLayer* get_animlayers_resolver();
    AnimationLayer* GetAnimOverlay_resolver(int index);
    int sequence_activity(int sequence);
    float get_max_desync_delta();
    float get_max_desync_delta_low();
    float get_max_desync_delta_mid();
    float get_max_desync_delta_air();
