...
void aim::run(CUserCmd* cmd)
{
	...
	const static auto cl_lagcompensation = m_cvar()->FindVar(crypt_str("cl_lagcompensation"));

	if (g_cfg.ragebot.anti_defensive)
...
void aim::automatic_revolver(CUserCmd* cmd)
{
	...
	auto server_time = TICKS_TO_TIME(g_ctx.globals.fixed_tickbase); // not need using backup tickbase..
...
bool aim::automatic_stop(CUserCmd* cmd)
{
	...
	auto max_speed = 0.10f * (g_ctx.globals.scoped ? weapon_info->flMaxPlayerSpeedAlt : weapon_info->flMaxPlayerSpeed);

	if (engineprediction::get().backup_data.velocity.Length2D() < max_speed)
		slowwalk::get().create_move(cmd, max_speed);
	else
	{
		Vector direction;
		Vector real_view;
...
void aim::scan(adjust_data* record, scan_data& data, const Vector& shoot_position, bool optimized)
{
	...
	for (auto& hitbox : hitboxes)
	{
		...
		{
			point.safe = record->bot || (hitbox_intersection(record->player, record->matrixes_data.negative, hitbox, shoot_position, point.point) && hitbox_intersection(record->player, record->matrixes_data.zero, hitbox, shoot_position, point.point) && hitbox_intersection(record->player, record->matrixes_data.positive, hitbox, shoot_position, point.point));
...
std::vector <int> aim::get_hitboxes(adjust_data* record, bool optimized)
{
	...
	bool ignore_limbs = g_cfg.ragebot.weapon[hooks::rage_weapon].rage_aimbot_ignore_limbs;
	...
	{
		if (g_cfg.ragebot.weapon[hooks::rage_weapon].bodyaimcond.at(0) && standing_detection)
		{
			hitboxes.emplace_back(HITBOX_RIGHT_THIGH);
			hitboxes.emplace_back(HITBOX_LEFT_THIGH);
			hitboxes.emplace_back(HITBOX_RIGHT_CALF);
		...
		if (g_cfg.ragebot.weapon[hooks::rage_weapon].bodyaimcond.at(1) && slow_walk_detection)
		{
			hitboxes.emplace_back(HITBOX_RIGHT_THIGH);
			hitboxes.emplace_back(HITBOX_LEFT_THIGH);
			hitboxes.emplace_back(HITBOX_RIGHT_CALF);
		...
		if (g_cfg.ragebot.weapon[hooks::rage_weapon].bodyaimcond.at(2) && record->player->m_iHealth() <= g_cfg.ragebot.weapon[hooks::rage_weapon].body_aim_health)
		{
			hitboxes.emplace_back(HITBOX_RIGHT_THIGH);
			hitboxes.emplace_back(HITBOX_LEFT_THIGH);
			hitboxes.emplace_back(HITBOX_RIGHT_CALF);
		...
		if (g_cfg.ragebot.weapon[hooks::rage_weapon].bodyaimcond.at(3) && !record->player->m_fFlags() & FL_ONGROUND)
		{
			hitboxes.emplace_back(HITBOX_RIGHT_THIGH);
			hitboxes.emplace_back(HITBOX_LEFT_THIGH);
			hitboxes.emplace_back(HITBOX_RIGHT_CALF);
...
void aim::fire(CUserCmd* cmd)
{
	...
	float final_hitchance = 0;
	float hitchance_amount = 0;

	auto autosniper = g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_SCAR20 || g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_G3SG1;
	int override_hitchance_config = g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitchance_override_amount;
	int hitchance_cfg = g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitchance_amount;
	...
	if (hitchance_amount)
	{
		...
		if (g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitchance_override && key_binds::get().get_key_bind_state(25 + g_ctx.globals.current_weapon))
			hitchance_amount = override_hitchance_config;
		else if (g_cfg.ragebot.nospread)
		{
			if (final_hitchance >= 20)
				final_hitchance = g_ctx.indexes.at(9), g_ctx.indexes.at(10);
		}
		else if (autosniper && g_ctx.globals.weapon->m_zoomLevel() == 0 && !final_hitchance == INT_MAX)
			hitchance_amount = hitchance_amount / 2;
		else if (autosniper && final_hitchance == INT_MAX)
			hitchance_amount = hitchance_amount / 3;
	...
	if (g_cfg.ragebot.nospread)
	    log << crypt_str("hitchance: nospread, ");
	else
...
float aim::hitchance(const Vector& aim_angle)
{
	...
	auto sniper = g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_AWP || g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_G3SG1 || g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_SCAR20 || g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_SSG08;
