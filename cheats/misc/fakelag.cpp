...
{
	if (g_cfg.antiaim.fakelag && !g_ctx.globals.exploits && g_ctx.globals.next_tickbase_shift <= 16)
	{
	...
	{
		if (g_cfg.ragebot.fake_duck_bypass)
		    max_choke = 8;
		else if (g_cfg.ragebot.fake_duck_nig)
			max_choke = 10;
		else
			max_choke = 14;
	...
	else
	{
		...
		if (g_cfg.antiaim.fakelag && g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_TASER)
		{
		...
		else if (g_cfg.antiaim.fakelag && g_cfg.antiaim.fakelag_disable_revolver && g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_REVOLVER)
		{
			max_choke = min(max_choke, 14);
		...
		else if (misc::get().double_tap_key && g_cfg.antiaim.fakelag && !misc::get().hide_shots_key && g_cfg.antiaim.double_tap_lag)
		{
			if (g_ctx.globals.weapon->get_max_tickbase_shift() > 15)
			    max_choke = 16 - g_ctx.globals.weapon->get_max_tickbase_shift() - 1;
			else
				max_choke = 16 - g_ctx.globals.weapon->get_max_tickbase_shift();
		...
		else if (!misc::get().double_tap_key && g_cfg.antiaim.fakelag && misc::get().hide_shots_key && g_cfg.antiaim.hide_shots_lag)
		{
			max_choke = min(max_choke, 7);
