...
void lagcompensation::update_player_animations(player_t* e)
{
...
#endif
	{
		...
		animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y - 58.0f); //-V807
		...
		setup_matrix(e, animlayers, ZERO_MATRIX);
		memcpy(animstate, &state, sizeof(c_baseplayeranimationstate));
		...
		setup_matrix(e, animlayers, POSITIVE_MATRIX);
		memcpy(animstate, &state, sizeof(c_baseplayeranimationstate));
	...
	setup_matrix(e, animlayers, MAIN_MATRIX);
