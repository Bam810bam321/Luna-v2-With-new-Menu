...
void local_animations::run(ClientFrameStage_t stage)
{
	...
	else if (stage == FRAME_RENDER_START)
	{
		...
		//zero pitch
		if (g_cfg.antiaim.pitch_zero_on_land && g_ctx.local()->get_animation_state()->m_bInHitGroundAnimation)
		{
			if (g_ctx.local()->get_animation_state()->m_flHeadHeightOrOffsetFromHittingGroundAnimation)
			{
				g_ctx.local()->m_flPoseParameter()[12] = 0.5;
			}
			else
			{
				g_ctx.local()->m_flPoseParameter()[12] = 1;
			}
		}
