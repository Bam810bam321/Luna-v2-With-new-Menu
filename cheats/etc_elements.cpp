...
bool e_elements::begin_child_ex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddText(c_menu::get().bold, 15.0f, parent_window->DC.CursorPos + ImVec2(16, 9), ImColor(147, 190, 66), name);

    // Build up name. If you need to append to a same child from multiple location in the ID stack, use BeginChild(ImGuiID id) with a stable value.
    char temp_window_name[256];
    if (name)
        ImFormatString(temp_window_name, IM_ARRAYSIZE(temp_window_name), "%s/%s_%08X", parent_window->Name, name, id);
    else
        ImFormatString(temp_window_name, IM_ARRAYSIZE(temp_window_name), "%s/%08X", parent_window->Name, id);
    ...
    if (g.NavActivateId == id && !(flags & ImGuiWindowFlags_NavFlattened) && (child_window->DC.NavLayerCurrentMask != 0 || child_window->DC.NavHasScroll))
    {
...
void e_elements::end_child()
{
    ...
    else
    {
        ...
        ItemSize(sz);
        if ((window->DC.NavLayerCurrentMask != 0 || window->DC.NavHasScroll) && !(window->Flags & ImGuiWindowFlags_NavFlattened))
        {
            ItemAdd(bb, window->ChildId);
            RenderNavHighlight(bb, window->ChildId);

            // When browsing a window that has no activable items (scroll only) we keep a highlight on the child (pass g.NavId to trick into always displaying)
            if (window->DC.NavLayerCurrentMask == 0 && window == g.NavWindow)
