...
void worldesp::molotov_timer(entity_t* entity)
{
	...
	render::get().circle_filled(screen_origin.x, screen_origin.y - size.y * 0.5f, 60, 20, Color(30,30,30,150));
	//int x, int y, int radius, int start_angle, int percent, int thickness, Color color
	render::get().draw_arc(screen_origin.x, screen_origin.y - size.y * 0.5f, 20, -90, (360 * factor), 2, (15, 15, 15, Color(191, 191, 191)));
	//
	...
	render::get().text(fonts[ESP], screen_origin.x, screen_origin.y - size.y * 0.5f + 12.0f, Color(191, 191, 191), HFONT_CENTERED_X | HFONT_CENTERED_Y, "");
	render::get().text(fonts[GRENADESINDICATOR], screen_origin.x + 1.0f, screen_origin.y - size.y * 0.5f + 2.0f,Color(200,200,200), HFONT_CENTERED_X | HFONT_CENTERED_Y, "l");
}
...
{
	/*if (!g_cfg.esp.grenade_proximity_warning) FIX THE CRASH IF THE FUTURE FOR THIS BUTIFULL INDICATOR
		return;
