...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	else if (!strcmp(event_name, crypt_str("bullet_impact")))
	{
		...
		if (user == m_engine()->GetLocalPlayer())
		{
			...
			if (g_cfg.esp.server_bullet_impacts)
				m_debugoverlay()->BoxOverlay(position, Vector(-2.0f, -2.0f, -2.0f), Vector(size, size, size), QAngle(0.0f, 0.0f, 0.0f), g_cfg.esp.server_bullet_impacts_color.r(), g_cfg.esp.server_bullet_impacts_color.g(), g_cfg.esp.server_bullet_impacts_color.b(), g_cfg.esp.server_bullet_impacts_color.a(), 4.0f);
			...
			if (current_shot && aim::get().last_target[current_shot->last_target].record.player->valid(true, false)) 
			{
				...
				if (aim::get().last_target[current_shot->last_target].data.point.safe)
				{
					...
					m_trace()->ClipRayToEntity(ray, MASK_SHOT_HULL | CONTENTS_HITBOX, aim::get().last_target[current_shot->last_target].record.player, &trace_positive);
				}

				auto hit = trace.hit_entity == aim::get().last_target[current_shot->last_target].record.player;

				if (aim::get().last_target[current_shot->last_target].data.point.safe)
					hit = hit && trace_negative.hit_entity == aim::get().last_target[current_shot->last_target].record.player && trace_zero.hit_entity == aim::get().last_target[current_shot->last_target].record.player && trace_positive.hit_entity == aim::get().last_target[current_shot->last_target].record.player;
	...
	else if (!strcmp(event_name, crypt_str("player_hurt")))
	{
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			...
			{
				m_surface()->PlaySound_(crypt_str("buttons/arena_switch_press_02.wav"));
			}
