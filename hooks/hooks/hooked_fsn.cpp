...
if (g_ctx.local()->is_alive())
	{
		...
		if (g_cfg.misc.rifkmodel)
			g_ctx.local()->m_flModelScale() = 0.4285717010498047;

		else
			g_ctx.local()->m_flModelScale() = 1;
	}
	...
	if (stage == FRAME_NET_UPDATE_POSTDATAUPDATE_START && g_ctx.local()->is_alive()) //-V522 
	{
	...
	if (stage == FRAME_RENDER_START)
	{
		if (g_cfg.esp.client_bullet_impacts)
		{
			...
			const auto& size = g_cfg.esp.bullet_impacts_width / 10.f;

			for (auto i = client_impact_list.Count(); i > last_count; --i)
				m_debugoverlay()->BoxOverlay(client_impact_list[i - 1].position, Vector(-2.0f, -2.0f, -2.0f), Vector(size, size, size), QAngle(0.0f, 0.0f, 0.0f), g_cfg.esp.client_bullet_impacts_color.r(), g_cfg.esp.client_bullet_impacts_color.g(), g_cfg.esp.client_bullet_impacts_color.b(), g_cfg.esp.client_bullet_impacts_color.a(), 4.0f);
		...
		ConVar* sway = m_cvar()->FindVar("cl_wpn_sway_scale");

		if (g_cfg.ragebot.nosway)
			sway->SetValue(0.0f);
		else
			sway->SetValue(1.6f);
				...
				if (!current_shot->hurt_player)
				{
					...
					else if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
					{
						...
						if (current_shot->occlusion)
							m_cvar()->ConsoleColorPrintf(Color(255, 255, 0), crypt_str("[Medusa.uno] Missed shot due to occlusion.\n"));
						else if (current_shot->shot_info.hitchance == 100)
							m_cvar()->ConsoleColorPrintf(Color(255, 0, 0), crypt_str("[Medusa.uno] Missed shot due to prediction.\n"));
						else
							m_cvar()->ConsoleColorPrintf(Color(255, 200, 0), crypt_str("[Medusa.uno] Missed shot due to spread.\n"));
					}
				}
			}
