...
namespace QGUI 
{
	...
	bool SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
	{
		...
		const ImRect frame_bb(window->DC.CursorPos + ImVec2(15,15), window->DC.CursorPos + ImVec2(w+0, 19));
		...
		if (!hovered)
		{
			window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 0, frame_bb.Min.y + 0), ImVec2(frame_bb.Max.x + 0, frame_bb.Max.y + 0), ImColor(67,67,67,255),0, 15); 
		}
		else
		{
			window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 0, frame_bb.Min.y + 0), ImVec2(frame_bb.Max.x + 0, frame_bb.Max.y + 0), ImColor(67,67,67,255),0, 15); 
		...
		if (!hovered)
		{
			window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x + 0, grab_bb.Min.y + 0), ImVec2(grab_bb.Max.x + 0, frame_bb.Max.y + 0), ImColor(7,142,246,255),0, 15);
		}
		else
		{
			window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x + 0, grab_bb.Min.y + 0), ImVec2(grab_bb.Max.x + 0, frame_bb.Max.y + 0), ImColor(7,142,246,255),0, 15); 
		}
        window->DrawList->AddRectFilled(ImVec2(grab_bb.Max.x - 2, grab_bb.Max.y - 8), ImVec2(grab_bb.Max.x + 2, grab_bb.Max.y + 8), ImColor(149, 149, 149));
		char value_buf[64];
		...
		ImGui::RenderText(ImVec2(frame_bb.Min.x, total_bb.Min.y + 9), label);
		ImGui::PopStyleColor();
