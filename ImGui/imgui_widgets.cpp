...
bool ImGui::ContentTab(const char* label, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImVec4 text_act = ImVec4(147/ 255.f, 190/ 255.f, 66/ 255.f, 1.f);
...
bool ImGui::MainTab(const char* label, ImFont* icon_font, const char* icon, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImVec4 icon_dis = ImVec4(202 / 255.f, 202 / 255.f, 202 / 255.f, 1.f);
    const ImVec4 icon_act = ImVec4(147/ 255.f, 190/ 255.f, 66/ 255.f, 1.f);
...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
    ...
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(147/ 255.f, 190/ 255.f, 66/ 255.f, it_hover->second)), 2.f * c_menu::get().dpi_scale);
...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg, bool use_icon, ImFont* icon_font, const char* icon_label)
{
    ...
    ImGui::GetWindowDrawList()->AddCircleFilled(it_circle->second.pos, it_circle->second.radius, GetColorU32(ImVec4(147/ 255.f, 190/ 255.f, 66/ 255.f, it_circle->second.alpha)), 60);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    // Render
    ImVec4 hover_dis = ImVec4(147/ 255.f, 190/ 255.f, 66/ 255.f, 1.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(147 / 255.0f, 190 / 255.0f, 66 / 255.0f, it_anim->second.selected_rect), 3.0f);
...
void ImGui::Spacing2(int spacing)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ItemSize(ImVec2(0, spacing));
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int* current_item)
{
    ...
    const ImVec2 arrow_size = c_menu::get().combo_arrow->CalcTextSizeA(9.0f, FLT_MAX, 0.0f, "A");
    ...
    PushFont(c_menu::get().combo_arrow); {
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    const ImVec4 hover_act = ImVec4(147/ 255.f, 190/ 255.f, 66/ 255.f, 1.f);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, bool minimum_damage, float power)
{
    ...
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, power, NULL, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y), ImColor(147, 190, 66), 5.0f);
